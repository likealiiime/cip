...
Number.implement({
	toInteger: function() { x = parseInt(this); return (x == NaN || x == null) ? null : x; }
});
String.implement({
	toInteger: function() { x = parseInt(this); return (x == NaN || x == null) ? null : x; }
});
...
CIApplication = new CIApplicationInstance();var CIRequestController = new Class({
	Extends: CIObject,
	...
	reset: function() {
		...
		this.synthesize({
			finishedCallback: function() { throw new Error('Called finishedCallback on ' + this.id + ', but it was undefined'); },
			activate: function() { throw new Error('Called activate on ' + this.id + ', but it was undefined'); }
		});
		return this;
	},
	getParamsObject: function() { return {}; },
	updateParamsHash: function(hash) { return hash; }
});
...
var CIStandardRequestController = new Class({
	...
	successCallback: function(request, method, response, json) {
		this.finishedCallback(method, response, json);
	}
});
...
var CIChunkRequestController = new Class({
	...
	getParamsObject: function() {
		return {
			'CIRequestChunkPosition': this.position,
			'CIRequestChunkSize': this.size
		};
	},
	
	updateParamsHash: function(params) {
		return params.set('CIRequestChunkPosition', this.position);
	},
	
	successCallback: function(request, method, response, json) {
		var total = response['CIRequestChunkTotal'];
		if (total == null) {
			request.fireEvent(CIEvent.RequestFailed);
			throw new Error(this.id + ' could not access response.CIRequestChunkTotal. ' + request.id + ' has failed.');
		}
		total = total.toInt();
		var chunks = (total / this.size).floor();
		if ((total % this.size) != 0) chunks++;
		this.buffer.extend($splat(response['CIRequestChunkCollection']));
		if (this.target)
			this.target['set'+this.property.capitalize()](this.buffer);
		else
			throw new Error('Cannot complete successCallback on ' + this.id + ' with null target.');
		
		if (chunks == this.position) {
			this.finishedCallback(method, this.buffer, '{ controller: "Called from ' + this.id + '"}');
		} else {
			this.position++;
			this.activate(this);
		}
...
var CIRequest = new Class({
	...
	*/
	initialize: function(options) {
		...
		$H(options).each(function(config, method) {
			...
			var controllerVar = method + 'Controller';
			this.synthesize([controllerVar]);
			this[controllerVar] = config.controller || new CIStandardRequestController();
			if (this.requestor) this[controllerVar].target = this.requestor;
		}.bind(this));
		this.forwarders = [];
	},
	...
	*/
	send: function(method, moreParams, argsObject) {
		...
		params.extend(controller.reset().getParamsObject());
		controller.finishedCallback = this._requestSuccess.bind(this);
		controller.activate = function(request) {
			params = this.updateParamsHash(params);
...
CIRequest.isValidHTTPMethod = function(method) {
	...
	toCssStylesObject: function(css, superview) {
		...
		if (this.width != null) {
			//styles.set('min-width', this.getWidth());
		...
		if (this.positioning) styles.set('position', this.positioning);
	...
	getHeight: function() { return this.height != null ? this.height.toInteger(this.superview) : 0; },
...
CIRect.TallestSubviewOf = function(superview) {
	// Construct an anonymous class to do the comparison and implement toInteger
...
CIRect.FlowPositioning = 'static';
CIRect.AbsolutePositioning = 'absolute';
CIRect.RelativePositioning = 'relative';var CIRequestable = new Class({
...
/*
	...
	Returns:
		CIElement
*/
function $E(tag, html) { return CIElement.make(tag, html); }kAlphabet = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
